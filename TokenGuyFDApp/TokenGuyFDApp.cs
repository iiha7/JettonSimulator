...
{
    public partial class CTokenGuyFDApp : Racon.CGenericFederate
    {
        ...
        #region Constructor
        public CTokenGuyFDApp(CSimulationManager parent) : this()
        {
        ...
        public bool SendMessage(int giveOrTake, string txt, int amount)
        {
            ...
            // Send interaction
            return (SendInteraction(interaction, "This amount I can provide"));
        }
        ...
        public override void FdAmb_StartRegistrationForObjectClassAdvisedHandler(object sender, HlaDeclarationManagementEventArgs data)
        {
            ...
            #region User Code
            if (data.ObjectClassHandle == Som.TokenGuyOC.Handle)
            {

                RegisterHlaObject(manager.TokenGuyObjects[0]);
            }
        ...
        public override void FdAmb_ObjectDiscoveredHandler(object sender, HlaObjectEventArgs data)
        {
            ...
            if (data.ClassHandle == Som.TokenGuyOC.Handle)
            {
                // Create and add a new player to the list
                CTokenGuyHlaObject newTokenGuy = new CTokenGuyHlaObject(data.ObjectInstance);
                newTokenGuy.Type = Som.TokenGuyOC;
                RequestAttributeValueUpdate(newTokenGuy, string.Empty);
                manager.TokenGuyObjects.Add(newTokenGuy);
                initialization = true;
        ...
        public override void FdAmb_AttributeValueUpdateRequestedHandler(object sender, HlaObjectEventArgs data)
        {
            ...
            #region User Code
            if (data.ObjectInstance.Handle == manager.TokenGuyObjects[0].Handle)
            {
                ...
                {
                    Program.noOfTokenGuys = 0;
                    foreach (CTokenGuyHlaObject tokenGuy in manager.TokenGuyObjects)
                    {
                        if (tokenGuy.TokenGuy.TokensToGive > 0)
                            Program.noOfTokenGuys++;
                    }
                    counter++;

                    if (Program.noOfTokenGuys > 0)
                    {

                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine(
                          "***************************************************************************\n"
                          + "                             " + "TokenGuy  " + counter + "is serving" + "\n"
                          + "***************************************************************************");

                        Console.WriteLine(DateTime.Now.TimeOfDay.Hours + ":" +
                            DateTime.Now.TimeOfDay.Minutes + ":" + DateTime.Now.TimeOfDay.Seconds);
                        Random rnd=new Random();
                        int tokensToGive = rnd.Next(1, 6);
                        manager.TokenGuyObjects[0].TokenGuy.tokensGiven(tokensToGive);
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("Token Guy gave tokens and only has : " + (manager.TokenGuyObjects[0].TokenGuy.TokensToGive).ToString());
                        Console.ResetColor();
                        SendMessage(0, "Token Guy gave tokens and only has : ", manager.TokenGuyObjects[0].TokenGuy.TokensToGive);
                    ...
                    else
                    {
                        ...
                        SendMessage(0, "No token guys left!!", 0);
                        Program.disconnected = true;
                    }
